...
namespace Microsoft.Azure.EventHubs.ServiceFabricProcessor
{
    ...
    {
        abstract public Task OpenAsync(PartitionContext context);

        abstract public Task CloseAsync(PartitionContext context, CloseReason reason);

        abstract public Task ProcessEventsAsync(PartitionContext context, IEnumerable<EventHubWrappers.IEventData> events);

        abstract public Task ProcessErrorAsync(PartitionContext context, Exception error);

        virtual public int GetLoadMetric(PartitionContext context)
        {
